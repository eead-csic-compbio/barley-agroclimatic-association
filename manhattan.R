...
#'
#' Copied from https://github.com/stephenturner/qqman (26/06/2018)
#'
#' Modified so that the font size of annotated SNPs can be controlled
#'
    ...
    if (!is.null(annotatePval)) {
        ...
        if (annotateTop == FALSE) {
            with(subset(d, P <= annotatePval), 
                 textxy(pos, -log10(P), offset = 0.625, labs = topHits$SNP, cex = snp_text_cex), ...)
        }
        else {
            ...
            textxy(topSNPs$pos, -log10(topSNPs$P), offset = 0.625, labs = topSNPs$SNP, cex = snp_text_cex, ...)
        }
    }  
    par(xpd = FALSE)
}
