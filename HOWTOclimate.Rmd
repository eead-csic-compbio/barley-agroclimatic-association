...
we need to build **ENVIRONFILE** file where each line is an environmental variable across populations, with tab-separated values and standardized after subtracting the mean and dividing by the standard deviation across populations. The variables should be listed in the same population order as they appear in the allele count files. 
...
```{r createSBCC_environfile}
  ...
  # now add PC vars (left out 09032017)
  #nrclimdata = merge( nrclimdata,
  #                    data.frame(id=climdata[w,1],pca$x[,1:5]),
  #                    by="id")
...
[*./maps/climatologies.RData*](./maps/climatologies_5km.RData).


We can also calculate maps of the first six components arising from the former
PCA.
...
if (!file.exists('./maps/climatologies_5km_pca.RData')) {
  ...
  eigenvec <- as.matrix(read.table('./raw/barley_climate_pca_eigenvectors.tsv',
                                   header=TRUE))
  ...
  kk <- sweep(sweep(as.data.frame(pcaclima), 2, scal[,'center'], '-'),
                2, scal[,'scale'], '/')
  pcaclima <- setValues(pcaclima, as.matrix(kk))
  #plot(pcaclima)
...
The results are stored on file
[*./maps/pcaMaps.RData*](./maps/pcaMaps.RData).
...
To produce a complete map showing the distribution of one snip on top of a
