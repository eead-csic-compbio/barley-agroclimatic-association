   ...
   ********* Implemented only for RANGE = 'A' or 'I' and IU - IL = N - 1   
   ...
   =====================================================================   
       ...
       Parameter adjustments */
        ...
        extern /* Subroutine */ int dstein_(integer *, doublereal *,
                                            doublereal *,
                                            integer *, doublereal *, integer *,
                                            integer *, doublereal *, integer *,
                                            doublereal *, integer *, integer *,
                                            integer *), dsterf_(integer *,
                                                                doublereal *,
                                                                doublereal *,
                                                                integer *);
        ...
        /* Function Body */
        ieeeok =
            ilaenv_(&c__10, "DSYEVR", "N", &c__1, &c__2, &c__3, &c__4,
                    (ftnlen) 6, (ftnlen) 1);
...
/* Computing MAX */
                i__1 = nb, i__2 =
                    ilaenv_(&c__1, "DORMTR", uplo, n, &c_n1, &c_n1, &c_n1,
                            (ftnlen) 6, (ftnlen) 1);
                nb = max(i__1, i__2);
/* Computing MAX */
                i__1 = (nb + 1) * *n;
...
/*     Scale matrix to allowable range, if necessary. */
        ...
        if (iscale == 1) {
                if (lower) {
                        ...
                        for (j = 1; j <= i__1; ++j) {
                                i__2 = *n - j + 1;
                                dscal_(&i__2, &sigma, &a[j + j * a_dim1],
                                       &c__1);
/* L10: */
                        }
                } else {
...
/*     Call DSYTRD to reduce symmetric matrix to tridiagonal form. */
        ...
        dsytrd_(uplo, n, &a[a_offset], lda, &work[indd], &work[inde],
       ...
       then call DSTERF or DSTEMR and DORMTR. */
        ...
        if ((alleig || (indeig && *il == 1 && *iu == *n)) && ieeeok == 1) {
                if (!wantz) {
                        ...
                        dstemr_(jobz, "A", n, &work[inddd], &work[indee], vl,
                                vu, il, iu, m, &w[1], &z__[z_offset], ldz, n,
                                &isuppz[1], &tryrac, &work[indwk], lwork,
                                &iwork[1], liwork, info);
...
/*        Apply orthogonal matrix used in reduction to tridiagonal   
          form to eigenvectors returned by DSTEIN. */
                        ...
                        if (wantz && *info == 0) {
                                ...
                                dormtr_("L", uplo, "N", n, m, &a[a_offset], lda,
                                        &work[indtau]
                                        , &z__[z_offset], ldz, &work[indwkn],
                                        &llwrkn, &iinfo);
                        }
                }
...
/*     Otherwise, call DSTEBZ and, if eigenvectors are desired, DSTEIN.   
       Also call DSTEBZ and DSTEIN if DSTEMR fails. */
        ...
        dstebz_(range, order, n, &vll, &vuu, il, iu, &abstll, &work[indd],
                &work[inde], m, &nsplit, &w[1], &iwork[indibl], &iwork[indisp],
                &work[indwk], &iwork[indiwo], info);

        if (wantz) {
                dstein_(n, &work[indd], &work[inde], m, &w[1], &iwork[indibl],
                        &iwork[indisp], &z__[z_offset], ldz, &work[indwk],
...
/* L40: */
                        ...
                        if (i__ != 0) {
                                w[i__] = w[j];
                                w[j] = tmp1;
                                dswap_(n, &z__[i__ * z_dim1 + 1], &c__1,
                                       &z__[j * z_dim1 + 1], &c__1);
                        }
/* L50: */
                }
