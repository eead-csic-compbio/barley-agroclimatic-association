...
/*  -- LAPACK auxiliary routine (version 3.1) --   
       ...
       Parameter adjustments */
        ...
        static doublereal t1, t2, t3, v2, v3, aa, ab, ba, bb, h11, h12, h21,
            h22, cs;
        static integer nh;
        ...
        extern /* Subroutine */ int drot_(integer *, doublereal *, integer *,
            ...
            dlanv2_(doublereal *, doublereal *, doublereal *, doublereal *,
                    ...
                    doublereal *, doublereal *), dlabad_(doublereal *,
                                                         doublereal *);
...
/*        Look for a single small subdiagonal element. */
                ...
                for (k = i__; k >= i__1; --k) {
                        if ((d__1 =
                             h__[k + (k - 1) * h_dim1], abs(d__1)) <= smlnum) {
                                goto L40;
                        }
                        tst = (d__1 =
                               h__[k - 1 + (k - 1) * h_dim1],
...
/* Computing MAX */
                                d__3 = (d__1 =
                                        h__[k + k * h_dim1], abs(d__1)), d__4 =
                                    (d__2 =
                                     h__[k - 1 + (k - 1) * h_dim1] - h__[k +
                                                                         k *
                                                                         h_dim1],
                                     abs(d__2));
                                aa = max(d__3, d__4);
...
/*              ==== real shifts (use only one of them)  ==== */
                                ...
                                rt1r = tr + rtdisc;
                                rt2r = tr - rtdisc;
                                if ((d__1 = rt1r - h22, abs(d__1)) <= (d__2 =
                                                                       rt2r -
                                                                       h22,
                                                                       abs
                                                                       (d__2)))
...
/*           Determine the effect of starting the double-shift QR   
             ...
             overflows and most underflows.) */
                        ...
                        s = (d__1 =
                             h__[m + m * h_dim1] - rt2r,
                             abs(d__1)) + abs(rt2i) + abs(h21s);
                        h21s = h__[m + 1 + m * h_dim1] / s;
                        v[0] =
                            h21s * h__[m + (m + 1) * h_dim1] +
                            (h__[m + m * h_dim1] -
                             rt1r) * ((h__[m + m * h_dim1] - rt2r) / s) -
                            rt1i * (rt2i / s);
                        v[1] =
                            h21s * (h__[m + m * h_dim1] +
                                    h__[m + 1 + (m + 1) * h_dim1]
...
/*           The first iteration of this loop determines a reflection G   
   ...
   Computing MIN */
                        ...
                        nr = min(i__2, i__3);
                        if (k > m) {
                                dcopy_(&nr, &h__[k + (k - 1) * h_dim1], &c__1,
                                       v, &c__1);
                        }
...
/*        H(I-1,I-2) is negligible: a pair of eigenvalues have converged.   
          ...
          and compute and store the eigenvalues. */
                ...
                dlanv2_(&h__[i__ - 1 + (i__ - 1) * h_dim1], &h__[i__ - 1 + i__ *
                        ...
                        &h__[i__ + i__ * h_dim1], &wr[i__ - 1], &wi[i__ - 1],
                        &wr[i__], &wi[i__], &cs, &sn);
...
/*           Apply the transformation to the rest of H. */
                        ...
                        if (i2 > i__) {
                                i__1 = i2 - i__;
                                drot_(&i__1, &h__[i__ - 1 + (i__ + 1) * h_dim1],
                                      ldh, &h__[i__ + (i__ + 1) * h_dim1], ldh,
                                      &cs, &sn);
                        }
                        i__1 = i__ - i1 - 1;
                        drot_(&i__1, &h__[i1 + (i__ - 1) * h_dim1], &c__1,
                              &h__[i1 + i__ * h_dim1], &c__1, &cs, &sn);
                }
                if (*wantz) {

/*           Apply the transformation to Z. */

                        drot_(&nz, &z__[*iloz + (i__ - 1) * z_dim1], &c__1,
                              &z__[*iloz + i__ * z_dim1], &c__1, &cs, &sn);
                }
        }
