#!/usr/bin/env perl
# Perl script to compute XtX for a (large) SNP file in parallel and then merge the result
# Bruno Contreras Moreira EEAD-CSIC Jul2017
...
if(!$ARGV[1])
{
  ...
  die "Example: bayenv2 -X -t -i input.tsv -p 20 -e env.tsv -n 87 -m matrix.txt -k 100000 -r 12345 -o test.env
\n\n";
}
else ## parse command-line arguments
{
  ...
  if($raw_command !~ m/-t/ || $raw_command !~ m/-X/)
  {
    die "# ERROR: command must include -t -X \n"
  }
...
foreach my $f (0 .. $#snp_files)
{
  ...
  else
  {
    ...
    # clean
    unlink($snp_file,$tmp_output_file,"$snp_file.freqs","$snp_file.bf");
  }
}
