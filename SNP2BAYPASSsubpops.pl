...
# similar to SNP2bayenv.pl
# prints SNPs in single line instead of twin lines

die "# usage: $0 <SNP.tsv> <sample order file> <subpop file> <outfile.tsv> [optional marker list]\n" if(!$ARGV[1]);

my ($SNPtsvfile,$order_file,$subpop_file,$outfile,$marker_list_file) = @ARGV;
...
my (%sample_order,%sample_subp,%subp_stats,%valid_column,@sorted_samples,%order2column,%list);
my ($sample,$col,$subp,$call,$marker,$allele1,$allele2,$MAF);

# 1) read valid samples and record their order 
...
# 2) read subpop file
open(SUBPOPS,'<',$subpop_file) || die "# cannot read $subpop_file\n";
while(<SUBPOPS>)
{
  # id	structure_cluster
  # SBCC001	1
  chomp;
  ($sample,$subp) = split(/\t/,$_);
  next if($sample eq 'id');
  $sample_subp{$sample} = $subp;
  $subp_stats{$subp}++;
}
close(SUBPOPS);

foreach $subp (sort keys(%subp_stats)) 
{
  printf(STDERR "# subpop %s contains %d accessions\n",
    $subp,$subp_stats{$subp});
} warn "\n";


# 3) parse SNPs and print them in BayPass format
...
while(<SNP>)
{
  ...
  else
  {
    ...
    # sum subpop allele freqs
    foreach $sample (@sorted_samples)
    {
      $subp = $sample_subp{$sample};
      $col = $order2column{$sample};
      
      if($freq{$allele1}{$col}){ $freq_subp{$subp}{$allele1}++ }
      if($freq{$allele2}{$col}){ $freq_subp{$subp}{$allele2}++ }
    }
    
    # print subpop allele freqs in a string
    foreach $subp (sort keys(%subp_stats))
    {
      printf(OUT "%d\t",$freq_subp{$subp}{$allele1} || 0);
      printf(OUT "%d\t",$freq_subp{$subp}{$allele2} || 0);
    }
    print OUT "\n";
